...
- `IdBasedPipeline` authors extend **`IdRuntimeParams`** instead — `RuntimeParams` plus the `ids` list the framework injects there. `ids` is not on `RuntimeParams` because for a plain `AbstractPipeline` it is an ordinary user parameter (`pipelines/api_example_pipeline.py` declares its own, with a default and an API-schema entry). Accordingly `params_from_typeddict` skips a framework key only when it is *inherited*: re-declaring one in your own TypedDict claims it back as a real parameter (`_declared_keys` in `core/abstract_pipeline.py`). Read `ids` with `.get("ids", [])` — the manager strips it from a job's params before dispatch, so it is gone worker-side, where `current_ids` is the key you want.
